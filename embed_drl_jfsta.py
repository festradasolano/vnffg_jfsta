...
class DRLJointFSTAEmbedding(DRLEmbedding):
    """
    TODO
    """
    ...
    def __update_vnffg(self, vnf_id: str):
        ...
        elif self.vnffg[-1] != vnf_id:
            self.vnffg.append(vnf_id)


class DRLJointFSTAEnv(DRLEnv):
    """
    TODO
    """
    MAX_VNF_INSTANCES = 3
    ...
    def reset(self) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        self.m2 = DRLJointFSTAEmbedding(network=self.network, vnfr=self.vnfr, node_pair_paths=self.node_pair_paths, max_hops=self.max_hops, seed=self.seed)
...
class DRLJointFSTARun(DRLRun):
    ...
    def __init__(self, m1: Embedding, network: Network, vnfr: VNFR, seed: int = None, experiment: int = 0):
        ...
        env = DRLJointFSTAEnv(m1=m1, network=network, vnfr=vnfr, seed=seed)
    ...
    def run_tuning(self, hparams: list[dict[str, Any]]) -> dict[str, Any]:
        """
        TODO
        """
        return super().run_tunning(hparams)
